...
| 指令                                     | 含义                               |
| ---------------------------------------- | ---------------------------------- |
| `add_compile_options`                    | 设置编译选项and pass macro to code |
...
| `BUILD_SHARED_LIBS`                             | create shared lib or static lib          |
| `XXX_FOUND`                                     | XXX是否找到                              |
| `XXX_DIR`                                       | XXX的查找路径                            |
...
| `CMAKE_SYSTEM_NAME`      | 当前操作系统的名称: "Linux", "Ubuntu" and so on |
...
**`CONFIG`**  
CMake searches for a file like `FindProtobuf.cmake` by default, searches for a file like `ProtobufConfig.cmake` or `protobuf-config.cmake` with this keyword.
...
2. **PUBLIC**: The include directories are used when compiling the target itself. They are also propagated to other targets that link against it, which means that corresponding include directories will be added to the compiler flags automatically.
...
CMake collects all source files of the target and groups them in the order they appear in the target's source list.
...
| Mode              | Behavior                                      |
| ----------------- | --------------------------------------------- |
| `BATCH` (default) | sequential batching                           |
| `GROUP`           | grouping controlled by `UNITY_GROUP` property |

```cmake
set_source_files_properties(a.cpp b.cpp PROPERTIES UNITY_GROUP group1)
set_source_files_properties(c.cpp d.cpp PROPERTIES UNITY_GROUP group2)

set_target_properties(my_target PROPERTIES UNITY_BUILD_BATCH_SIZE 4)
