...
原因分析: 编译`main_node.cpp`时没有指定要链接定义WheelOdom的cpp文件; 模板类的成员函数的声明和定义分开了, 使模板类在实例化时不能使用类的模板，而是使用定义时的模板，这会产生构造和析构函数未定义的效果;
...
```bash
error: passing 'const custom::PointWithId' as 'this' argument discards qualifiers [-fpermissive]
```
...
```bash
there are no arguments to 'findLeaf' that depend on a template parameter, so declaration of 'findLeaf' must be declaration.
```
...
```bash
error: assignment of member 'ceres::Solver::Options::parameter_tolerance' in read-only object
```

`options` object is declared as `const` (or is being passed as `const&`). Since parameter_tolerance is a normal double member of `ceres::Solver::Options`, you can only modify it on a non-const object.
...
This error is emitted by the C++ compiler when it encounters a floating-point literal that uses the f suffix (for float) but is being parsed in a language mode that does not support C++11 user-defined literal syntax.
...
1. Division by zero like `int c = a / 0;`
2. Invalid floating-point operations, such as `double x = std::sqrt(-1.0);`
3. Modulo by zero like `int x = 10 % 0; `
4. index is beyond size of vector

##### 101

```bash
math cannot parse the expression "/1000"
```

replace `find_package(Protobuf REQUIRED)` with `find_package(Protobuf CONFIG REQUIRED)`

##### 102

`std::max(a, b)` is reported as a grammar/syntax error when compiling with Visual Studio 2022 on Windows.

On Windows, the header defines macros:
...
These macros replace the token max before the compiler sees the C++ code, which breaks std::max.

1st solution: disable the macros
...
2nd solution: undefine the macros
...
3rd solution: use parentheses trick
...
several members are declared using same type. but some members can be declared, but some members can't be declared. then the problem is usually not the type itself, but the specific member name or the syntax around that declaration.

It's possible that the member name conflicts with a macro. This is very common on Windows, especially with VS 2022 and Windows headers. For example, names like these may already be macros or widely used identifiers:
...
But here is another important issue. The `static` here means internal linkage for every translation unit. Each `.cpp` gets a separate large, which is almost never what you want. Prefer:
...
same code, in linux and gcc, template can be deduced. but on windows and vs 2022. it can't be deduced.

In practice, this usually means GCC accepted something through a different deduction path, an extension, or stronger support in that case, while MSVC needs the code written more explicitly.
...
`boost::make_shared` is defined in:
...
`___chkstk_ms` is a stack probing function used by Microsoft toolchains to safely allocate large stack frames. This almost always indicates a toolchain mismatch, especially between:

1. MSVC vs MinGW
2. Different runtime libraries
3. Different compiler targets (clang-cl vs clang vs gcc)
4. Mixing static libraries built with different compilers
...
```C++
PyTuple_SetItem(args, 0, PyFloat_FromDouble(n_rows));
PyTuple_SetItem(args, 1, PyFloat_FromDouble(n_cols));
...
```C++
PyTuple_SetItem(args, 0, PyLong_FromDouble(n_rows));
PyTuple_SetItem(args, 1, PyLong_FromDouble(n_cols));
...
##### 33 access non-exist json element
...
```bash
C++ exception with description "vector::_M_range_insert" thrown
```

`vector::_M_range_insert` almost always means iterator misuse, not memory exhaustion or capacity issues.

##### 36 assign into outbound position
...
operator[] does not perform bounds checking. The standard explicitly allows this access to be unchecked. As a result, the program may:

1. write to memory that happens to be accessible,
2. overwrite unrelated data,
3. silently corrupt internal state,
4. or crash immediately.

Accessing `vec[0]` in that state causes undefined behavior. This may:

1. crash,
2. appear to “work”,
3. corrupt memory,
4. or fail later in unrelated code.

in another word, it is entirely possible that the code does not crash immediately even when vec is empty.
...
for(every point)
{
  while(true)
  {
    ...
    start a thread to cluster
    break;
  }
  if(there's waiting cluster)
  {
    start a thread to process cluster
  }
}
```
