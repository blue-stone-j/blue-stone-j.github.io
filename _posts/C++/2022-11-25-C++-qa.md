...
It's necessary to add omp support when compile and install clang. If not, it will happen when performing `cmake`. Solve it by compile clang with omp support and install clang.
...
```
`obj` is a const varible but there is no gurantee that `getCloud` won't change `obj`. Add this gurantee to code like below
```C++
class Obj
{
  Cloud cloud;
...
2. Explicit Instantiation: use explicit instantiation in the implemention file to tell the compiler which template instances to instantiate.
---
...
Finally, I found that a varible should be declared in a class, but I moved it out of class to make it a global varible wrongly. It works.
