...
### find out which image a Docker container originated from
...
### reclaim host disk space from the VHDX

1. Enter Docker’s WSL distro()

```bash
wsl -d docker-desktop
```

2. Zero the free space
...
If you got error like `dd: invalid argument 'progress' to 'status'`, use `dd if=/dev/zero of=/zero.fill bs=1M || true`, which remove `status=progress`. The command will eventually terminate with `dd: write error: No space left on device`.

* `dd`: A low-level Unix utility that copies raw bytes from an input to an output.
* `if=/dev/zero`: `/dev/zero` is a special Linux device. It produces an infinite stream of zero bytes (`0x00`).
* `of=/zero.fill`: Writes those zeros into a file named `/zero.fill`. The file is created in the root filesystem.
* `bs=1M`: Block size = 1 `megabyte`. Improves performance (fewer syscalls). Has no effect on correctness.
* `|| true`: Prevents the shell from treating the error as fatal. Without it, some shells would abort the script.
* `sync`: Forces all buffered writes to be flushed to disk. Ensures zeros are physically written into the `VHDX`. Because WSL uses write caching. Without sync, Windows might compact before data is written.
* `rm /zero.fill`: Deletes the file you just created. `ext4` marks those blocks as free again. Now, the blocks contain zeros, not random data. From Linux’s point of view, disk is free again. From Windows’ point of view, those blocks are zero-filled and discardable.

1. Shut down Docker and WSL completely
...
Method mentioned above is the best. Below is another method, in case the last method(`Optimize-VHD`) isn't available.
...
2. Click the ⚙ Settings (top right)
3. Go to Docker Engine: This is the JSON configuration for the daemon
4. Add (or modify) the dns field.
...
6. Then start a container. For linux container, file `/etc/resolv.conf` will be reset as `nameserver 192.168.100.7`
...
```bash
Failed to connect to the remote extesion host server (Error: WebSocket close with status code 1006)
```
