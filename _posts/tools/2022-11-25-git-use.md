...
Local configuration will be save in file `.git/config`.
...
```bash
# not treat future file mode differences as meaningful
...
# download with limited commits
git clone --depth 10 https://github.com/user/repository.git

# clone the submodules along with the main repository
...
# set fetch and push individually:
git remote set-url --fetch origin https://new-url/repo.git
...
# create a commit even if there are no changes staged.
...
# --amend option: update the commit message or add additional changes that were missed in the previous commit
...
If you run `git reset --soft HEAD^` by mistake, you can run `git reflog` to look for the commit message of the reset commit (it will have a record like `HEAD@{1}` showing commit before the reset) and use that message to recommit. Or you can run `git reset --soft HEAD@{1}` to fully undo the git reset(move HEAD back to the original commit). However, the uncommitted changes can't be recovered.

If the deleted commit does not appear in the reflog, recovery is not possible through Git.
...
`git stash` moves uncommitted modifications into a temporary storage area inside .git. After stashing:

1. Working directory becomes clean.
2. Files revert to the last committed version.

A stash is stored inside local `.git` directory. It is not part of any branch, commit, or history that gets pushed. Therefore, running git push does not include stashes.

Every stash entry is actually a pair (sometimes three) of hidden commits stored in `.git/objects/`. Git does not create a separate file or folder for stashes. They live in the same place as normal commits.

A stash entry (`stash@{n}`) actually includes:

1. WIP commit (work-tree changes)
2. Index commit (staged changes)
3. Untracked files commit (optional, when using `--include-untracked`)
...
move or rename the file after creating the stash, if Git can identify that the file was moved (content match), it applies the stashed patch to the new location.

```bash
# Apply (keep stash entry in list)
...
```bash
# Delete a chosen entry (for example, stash@{1})
...
# This tag will now point to the latest commit in the current branch if commit isn't specified
...
| Aspect                 | LF                                 | CRLF                                   |
| ---------------------- | ---------------------------------- | -------------------------------------- |
| Linux tools            | ✔ Native                           | ⚠ Often tolerated but may cause issues |
| macOS                  | ✔ Native                           | ⚠ Sometimes visible as `^M`            |
| Windows editors        | ✔ Supported by most modern editors | ✔ Native                               |
| Git repositories       | ✔ Preferred                        | ⚠ Usually normalized to LF             |
| Compilers/build tools  | ✔ Safe                             | ⚠ Sometimes problematic                |
| Shell scripts (`bash`) | ✔ Required                         | ❌ Often breaks execution               |
...
### tag current changes without impact on current branch

##### solution 1: use stash

```bash
# use git stash as a snapshot
git stash push -u -m "snapshot-2026-04-07"
# then you can run "git stash pop" to restore and keep editing
```

##### solution 2: tag using stash

This does not include untracked files.

```bash
git stash create "snapshot" # This returns a commit hash.
git tag snapshot-2026-04-07 <hash>
```

##### solution 3: use git reset
...
Check with `git status`, if say something like
