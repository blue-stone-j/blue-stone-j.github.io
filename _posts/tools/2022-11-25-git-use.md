...
`.gitignore` is another similiar way to realize it. It's 一个文本文件，用于告诉 Git 哪些文件或目录是可以忽略的，不应该被包括在版本控制中。
...
run commond below to 从Git的工作目录中删除索引文件并重新生成一个新的索引文件
...
There isn't origin in cuttent repository. Use `git remote add origin https://github.com/blue-stone-j/geometry.git` instead.
