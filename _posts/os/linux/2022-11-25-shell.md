...
```bash
find /path/to/directory -type f -perm 755 -exec chmod 644 {} +
```

* `-type f` → only regular files
* `-perm 755` → Matches files whose permission is exactly 755.
* `chmod 644` → remove executable bit
* `{} +` → apply to many files efficiently
