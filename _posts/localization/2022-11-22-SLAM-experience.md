...
* vins紧耦合gps或者2D lidar: <https://bittly.sigechen.com/>  
...
单线激光雷达，用cartographer，有的带有视觉融合。carto建图，基于高精地图使用`amcl`定位，carto实时定位比较消耗资源。cartographer，建图可以，走的时候，经常会遇到问题。还有就是断电后，激光的好像很难恢复。
...
用excel表格弄出主车和目标车的位置信息和速度、相对速度信息，用`Vissim`仿真出两辆车的行驶轨迹动态图像。
...
Cartographer没有轮速计的情况，就建图来说差不多。不过需要先去畸变
